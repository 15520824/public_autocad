 ...
 *  (at your option) any lnger version.
 *
...
(function (root, factory) {
        ...
        if (this.type.toLowerCase() === 'box') {
            ...
            for (i in cs) {
                if (cs.hasOwnProperty(i)) {
                    json.bbox = json.bbox.concat([cs[i].lng, cs[i].lat]);
                }
            }
            ...
            if (cs.hasOwnProperty(i)) {
                ...
                if (Wkt.isArray(cs[i])) {
                            ...
                            if (Wkt.isArray(cs[i][j])) { // MULTIPOLYGONS
                                ...
                                for (k in cs[i][j]) {
                                    if (cs[i][j].hasOwnProperty(k)) {
                                        ring.push([cs[i][j][k].lng, cs[i][j][k].lat]);
                                    }
                                }
                                ...
                                if (cs[i].length > 1) {
                                    rings.push([cs[i][j].lng, cs[i][j].y]);

                                } else { // MULTIPOINTS
                                    rings = rings.concat([cs[i][j].lng, cs[i][j].lat]);
                                }
                            }
                        }
                    }
                    ...
                    if (cs.length > 1) { // For LINESTRING type
                        json.coordinates.push([cs[i].lng, cs[i].lat]);

                    } else { // For POINT type
                        json.coordinates = json.coordinates.concat([cs[i].lng, cs[i].lat]);
                    }
                }
    ...
    Wkt.Wkt.prototype.extract = {
        ...
        point: function (point) {
            return String(point.lng) + this.delimiter + String(point.lat);
        },
    ...
    Wkt.Wkt.prototype.ingest = {
        ...
        point: function (str) {
            ...
            return [{ // ...Search for numeric substrings
                lng: parseFloat(this.regExes.numeric.exec(coords[0])[0]),
                lat: parseFloat(this.regExes.numeric.exec(coords[1])[0])
            }];
        },
            ...
            for (i = 0; i < rings.length; i += 1) {
                ...
                for (j = 0; j < ring.length; j += 1) {
                    ...
                    if (split.length === 2) {
                        ...
                        //now push
                        subcomponents.push({
                            lng: parseFloat(x_cord),
                            lat: parseFloat(y_cord)
                        });
                    }
                }
